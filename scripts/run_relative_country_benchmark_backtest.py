...
OUTPUT_ROOT = ROOT / "output" / "relative_country_benchmark_backtest_2018_2026"
...
GAMMAS = (0.8, 1.0, 1.2, 1.4, 1.6, 2.0)
...
def season_benchmark(folder: Path, config: AOEuropeanEloConfig) -> float:
    """The strongest weighted country score of one season."""
...
def relative_seed_frame(
    config: AOEuropeanEloConfig, surprise: pd.DataFrame, relative: bool
) -> pd.DataFrame:
    ...
    for folder in sorted(STATIC_DATA_ROOT.glob("20??-??")):
        ...
        seasonal = config
        if relative:
            anchor = season_benchmark(folder, config)
            seasonal = replace(config, country_strength_benchmark=anchor)
            anchors.append({"season": season, "benchmark": anchor})
        ...
        if relative:
            # The anchor is the season maximum, so nothing can exceed it and the
            # clip must stay inert. If it ever fires the anchor is wrong.
...
def main() -> None:
    ...
    for index, candidate in enumerate(configs, start=1):
        seeds = relative_seed_frame(candidate.config, surprise, candidate.relative)
        if candidate.relative and not anchors:
            anchors = seeds.attrs["anchors"]
        rating_map = {
