...
"""Walk-forward test of a constant European blend weight.
...
OUTPUT_ROOT = ROOT / "output" / "constant_exposure_backtest_2018_2026"
...
CONSTANT_WEIGHTS = (0.40, 0.50, 0.55, 0.60, 0.65, 0.70)
...
def candidate_grid() -> tuple[Candidate, ...]:
    return (
        ...
        *(
            Candidate(f"flat{weight:g}", weight, BASELINE_CONFIG)
            for weight in CONSTANT_WEIGHTS
        ),
    )
...
def seeds_for(
    seeds: pd.DataFrame, volume: pd.DataFrame, weight: float | None
) -> pd.DataFrame:
    """Give every club with European evidence the same blend weight.

    The exposure layer exists to weight each club's European Prior by how much
    European football it has played. This candidate removes that per-club
    variation entirely and asks whether the layer earns its place. A club with
    no European evidence keeps weight zero: its European Prior is the 500 base
    placeholder rather than evidence, so any weight on it would be corruption.
    """
    if weight is None:
        return seeds
    exposure = pd.Series(float(weight), index=seeds.index)
    exposure = exposure.where(seeds["european_exposure"] > 0.0, 0.0)
    return seeds.assign(european_exposure=exposure)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Measure a constant European blend weight"
    )
    ...
    pd.DataFrame(reach_rows).to_csv(output / "weight_reach.csv", index=False)
