...
"""Walk-forward test of a floor on the European blend weight.
...
OUTPUT_ROOT = ROOT / "output" / "exposure_floor_backtest_2018_2026"
...
FLOORS = (0.20, 0.30, 0.35, 0.40, 0.45, 0.50)
...
def candidate_grid() -> tuple[Candidate, ...]:
    return (
        ...
        *(
            Candidate(f"floor{floor:g}", floor, BASELINE_CONFIG)
            for floor in FLOORS
        ),
    )
...
def seeds_for(
    seeds: pd.DataFrame, volume: pd.DataFrame, floor: float | None
) -> pd.DataFrame:
    """Mirror the 0.65 cap with a floor on the same weight.

    Production caps the European share at 0.65, so a club at the cap is scored
    65% European / 35% domestic. Nothing bounds the other end: a club at 0.20
    is scored 80% domestic, a far stronger claim about domestic evidence than
    the model ever makes for its best-evidenced clubs, and one nobody chose.

    The floor cannot touch the capped region by construction, which is what
    makes it testable without disturbing the frozen 0.65. A club with no
    European evidence keeps weight zero: its European Prior is the 500 base
    placeholder rather than evidence.
    """
    if floor is None:
        return seeds
    raw = seeds["european_exposure"]
    exposure = raw.clip(lower=float(floor))
    exposure = exposure.where(raw > 0.0, 0.0)
    return seeds.assign(european_exposure=exposure)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Measure a floor on the European blend weight"
    )
    ...
    pd.DataFrame(reach_rows).to_csv(output / "floor_reach.csv", index=False)
