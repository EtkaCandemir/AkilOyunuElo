...
OUTPUT_ROOT = ROOT / "output" / "anchor_headroom_backtest_2018_2026"
...
# `max` pins the strongest league at exactly 1.0, which is what lets its clubs
# sit still while every other league falls - a bias worth 3.42 ranks. Setting
# the anchor above the maximum leaves nobody at 1.0 and nobody pinned, while
# still making the clip impossible. The headroom values say how much slack that
# needs. `top3` and `p95` are kept as the alternative escape: they break the
# pinning by moving the anchor off any single league, at the cost of clipping.
ANCHORS = ("max", "max105", "max110", "max120", "top3", "p95")
    ...
    return (
        ...
        *(
            Candidate(
                ...
                "RELATIVE_BENCHMARK",
                gamma,
                replace(ACTIVE, gamma=gamma, achievement_alpha=0.0),
                True,
                anchor,
            )
...
def season_benchmark(
    ...
    elif anchor.startswith("max1"):
        value = float(scores.max()) * (1.0 + int(anchor[3:]) / 100.0 - 1.0)
    elif anchor == "top3":
...
def relative_seed_frame(
    ...
    for folder in sorted(STATIC_DATA_ROOT.glob("20??-??")):
        ...
        if relative and anchor.startswith("max"):
