...
"""Post-hoc discovery study of competition-specific exposure tails.
...
Each candidate applies the CAP_TAIL exposure mapping to team-season seed rows
from exactly one entry competition (UCL, UEL or UECL).  Every other seed row
uses the production rating, so this study tests whether harm in other cups
masked a UCL-specific signal in the earlier global mapping.
"""
...
OUTPUT_ROOT = (
    ROOT / "output" / "competition_exposure_tail_discovery_2018_2026"
)
...
def competition_specific_seed_frame(
    ...
    """Apply the tail only to the candidate's entry-competition seed rows."""
    ...
    if not tail[["season", "team_id", "competition"]].equals(
        production[["season", "team_id", "competition"]]
    ):
        raise ValueError("Tail and production seed frames are not row-aligned")
    mask = production["competition"].eq(candidate.target_competition)
    result = production.copy()
    candidate_columns = [
    ...
    if not result.loc[mask, "competition"].eq(candidate.target_competition).all():
        raise ValueError("Competition-tail target mask is inconsistent")
    return result
...
def build_report(
    ...
    return "\n".join(
        [
            "# Competition-Specific Exposure Tail Discovery",
            "",
            "> **GEÇERSİZ / ZAMAN SIZINTILI / PROMOTION KANITI DEĞİL.** Tarihsel "
            "`competition` etiketi takımın UCL elemesini geçip geçmediğini sonradan "
            "kodluyor. Bu etiket sezon başlangıcı AO First hesabında kullanılamaz.",
            "",
            "## Sonuç",
            "",
            f"- Karar etiketi: `{decision['decision']}`",
            "- Promotion için uygun: `False`",
            f"- Sonuç-koşullu qualifier etiketi: "
            ...
            "sorusu aynı veri üzerindeki önceki segment sonuçlarından doğdu ve aday "
            "anahtarı tahmin anında bilinmeyen qualification sonucunu kullandı. Bu "
            "nedenle iyileşme nedensel model kanıtı değildir.",
            "",
            "## Zaman sızıntısı kanıtı",
            "",
            "`entry_round=ch/Q*` veya `CL-Q*` takımın UCL qualifying'e girdiğini "
            "gösterir. Aynı satırın `competition=UEL/UECL` olması ancak sonraki "
            "eleme sonucundan sonra bilinebilir.",
            "",
            ...
            "- Her challenger yalnız UCL, UEL veya UECL entry-competition seed "
            ...
            "## Kanıt sınırı",
            "",
            "Bu sonuç model seçiminde kullanılamaz: hem post-hoc'tur hem de aday "
            "switch'i gelecekteki qualification sonucuna bağlıdır. Accuracy için "
            "ayrı güven aralığı hesaplanmaz. Production contract, config ve artifact "
            "zinciri değiştirilmemiştir.",
            "",
            "## Yeniden üretme",
            "",
            "`python3 scripts/run_competition_exposure_tail_backtest.py`",
        ]
    ) + "\n"
...
def main() -> None:
    ...
    decision = {
        "decision": "INVALID_OUTCOME_CONDITIONED_COMPETITION_LABEL",
        "eligible_for_promotion": False,
        "timing_leakage_detected": True,
    ...
    print("INVALID RESULT — OUTCOME-CONDITIONED COMPETITION LABEL", flush=True)
