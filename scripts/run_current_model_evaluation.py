    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"- Tarihsel nested `ML_POISSON_ENSEMBLE` kolu Brier `{prediction_evidence['pooled_brier']:.6f}`, log-loss `{prediction_evidence['pooled_log_loss']:.6f}`, accuracy `{prediction_evidence['pooled_accuracy']:.4f}` üretmiştir. Bu kol her fold için Poisson kaynağı ve ağırlığı seçer; sabit production karışımının birebir replay ölçümü değildir. Kaynak: `reports/production_prediction/model_comparison.csv` ve `reports/production_prediction/fold_selections.csv`.",
