...
"""Does the 0.65 blend weight still hold once the Domestic Prior changes shape?

    AO First = Domestic + w * (European - Domestic),   w = min(exposure, cap)

The cap is a statement about how far to trust one estimator over the other, and
the right answer depends on their relative quality. Every sweep that settled on
0.65 - the original exposure study and the participation interaction that
re-confirmed it - ran against the Domestic Prior as production builds it today:
gamma 0.80, achievement_alpha 0.40, a fixed benchmark of 25.

The candidate from this session rebuilds that side. It anchors the benchmark on
each season's strongest league, raises gamma to 1.2 and drops alpha to 0.0,
which spreads league strength and ties domestic achievement entirely to the
league it was earned in. A domestic estimate that carries different information
has no reason to deserve the same 35% share.

Both arms are swept over the same cap range so the question is answered in the
only way that means anything: not "is 0.65 good" but "does the optimum move".
...
from scripts.run_anchor_headroom_backtest import (  # noqa: E402
    Candidate,
    relative_seed_frame,
)
...
OUTPUT_ROOT = ROOT / "output" / "exposure_cap_after_domestic_shape_2018_2026"
ACTIVE = AOEuropeanEloConfig.active()
# Brackets the live 0.65 on both sides. A wider range is pointless here: the
# cap has been swept from 0.40 to 0.85 twice already and both times the surface
# was flat well outside this window.
CAPS = (0.55, 0.60, 0.65, 0.70, 0.75)
# The Domestic Prior candidate this run exists to test against.
CANDIDATE_GAMMA = 1.2
CANDIDATE_ALPHA = 0.0


def candidate_grid() -> tuple[Candidate, ...]:
    rows = [Candidate(BASELINE_KEY, "BASELINE", 0.0, ACTIVE, False, "max")]
    for cap in CAPS:
        rows.append(
            Candidate(
                f"oldDP_cap{cap:g}",
                "PRODUCTION_DOMESTIC",
                cap,
                replace(ACTIVE, max_european_exposure=cap),
                False,
                "max",
            )
        )
        rows.append(
            Candidate(
                ...
                replace(
                    ACTIVE,
                    max_european_exposure=cap,
                    gamma=CANDIDATE_GAMMA,
                    achievement_alpha=CANDIDATE_ALPHA,
                ),
                True,
                "max",
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Sweep the exposure cap under both Domestic Prior shapes"
    )
    ...
    for index, candidate in enumerate(configs, start=1):
        ...
        for row in rows:
            row["cap"] = float(candidate.config.max_european_exposure)
            row["domestic_shape"] = candidate.axis
    ...
    lookup = (
        surface[["candidate_key", "cap", "domestic_shape"]]
    ...
    # The question this run asks is where each arm's own optimum sits, so record
    # both rather than only the winner across the pooled grid.
    best = {}
    for shape in ("PRODUCTION_DOMESTIC", "CANDIDATE_DOMESTIC"):
        arm = summary_frame.loc[summary_frame["domestic_shape"].eq(shape)]
        if not arm.empty:
            row = arm.sort_values("brier_1x2").iloc[0]
            best[shape] = {"key": str(row["candidate_key"]), "cap": float(row["cap"])}
    payload = {
        "decision": (
            "PROMOTE_CANDIDATE"
            if brier_wins >= 4
        ...
        "best_cap_per_domestic_shape": best,
    ...
    print(f"Best cap per shape: {best}")
