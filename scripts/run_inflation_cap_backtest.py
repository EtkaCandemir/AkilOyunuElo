...
"""Walk-forward test of a cap on the participation inflation multiplier.
...
OUTPUT_ROOT = ROOT / "output" / "inflation_cap_backtest_2018_2026"
...
INFLATION_CAPS = (1.10, 1.15, 1.20, 1.30, 1.50, 2.00)
...
def candidate_grid() -> tuple[Candidate, ...]:
    return (
        ...
        *(
            Candidate(f"infcap{cap:g}", cap, BASELINE_CONFIG)
            for cap in INFLATION_CAPS
        ),
    )
...
def seeds_for(
    seeds: pd.DataFrame, volume: pd.DataFrame, cap: float | None
) -> pd.DataFrame:
    """Cap how far a thin European record can be scaled up.

    Production normalizes history over participated seasons:

        rate = H * (1 + k) / (pw + k),   k = 0.20

    That multiplier reaches 4.44x for the thinnest record, which is what lets a
    club with three European seasons outrank one with five and twice the raw
    UEFA points. Raising `k` was measured and rejected: it pulls every club
    toward the raw sum and every metric got monotonically worse. This candidate
    leaves the curve alone and clips only its extreme, so a club already at
    full participation (multiplier 1.00) cannot move by construction.

    The clip is applied through the participation weight the normalization
    reads, not through `european_exposure`, so the blend weight is untouched:
    solving `(1+k)/(pw' + k) = cap` gives `pw' = (1+k)/cap - k`.
    """
    if cap is None:
        return seeds
    k = BASELINE_CONFIG.participation_shrinkage
    floor = (1.0 + k) / float(cap) - k
    played = seeds["weighted_season_exposure"]
    clipped = played.clip(lower=floor)
    # A club that never played keeps zero: its European Prior is the base
    # placeholder, and lifting its participation weight would invent evidence.
    clipped = clipped.where(played > 0.0, 0.0)
    return seeds.assign(weighted_season_exposure=clipped)


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Measure a cap on the participation inflation multiplier"
    )
    ...
    pd.DataFrame(reach_rows).to_csv(output / "cap_reach.csv", index=False)
