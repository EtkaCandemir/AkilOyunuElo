...
OUTPUT_ROOT = ROOT / "output" / "domestic_prior_axis_backtest_2018_2026"
...
# Multiplicative on the live component so the baseline is exactly 1.0 and the
# steps are symmetric around it.
COMPONENT_SCALES = (0.85, 1.15, 1.30)
...
def candidate_grid() -> tuple[Candidate, ...]:
    return (
        ...
        *(
            Candidate(f"gamma{value:g}", "GAMMA", value, replace(ACTIVE, gamma=value))
            for value in GAMMAS
        ),
        *(
            Candidate(
                ...
                replace(
                    ACTIVE,
                    domestic_league_component=ACTIVE.domestic_league_component * scale,
                ),
            )
            for scale in COMPONENT_SCALES
        ),
        *(
            Candidate(
                f"achievement{scale:g}",
                "ACHIEVEMENT_COMPONENT",
                scale,
                replace(
                    ACTIVE,
                    domestic_achievement_component=(
                        ACTIVE.domestic_achievement_component * scale
                    ),
                ),
            )
            for scale in COMPONENT_SCALES
